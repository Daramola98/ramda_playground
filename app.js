...
// Check if value of the input is Alphanumeric. Return true if it is, false otherwise.
const isNonAlphaNumeric = R.compose(isNotZero, R.length(), R.match(/[^a-zA-Z0-9]/g));
...
const compareTags = R.comparator((a, b) => a[1] > b[1]);

const compareDates = R.comparator((a, b) => a['date_taken'] < b['date_taken']);
...
const imageCount = (flickrData) => R.length(flickrImageData(flickrData));

const alphaNumericTagsUniq = R.compose(R.sort(compareStrings), R.uniq, R.filter(isAlphaNumeric), R.map(R.toLower), flickrTags);

const nonAlphaNumericTags = R.compose(R.filter(isNonAlphaNumeric), flickrTags)

const avgTitleLength = R.compose(R.mean, R.map(R.length()), flickrTitles);

const getRankings = R.compose(R.sort(compareTags), R.toPairs, R.countBy(R.toLower), flickrTags);

const filterItemsObject = R.map(R.pick(['title', 'date_taken']));

const sortByDateTaken = R.sort(compareDates)

const commonTagByRank = rank => (flickrData) => R.nth(rank, getRankings(flickrData))[0];

const oldestPhotoTitle = (flickrData) => R.nth(0, R.identity(R.compose(sortByDateTaken, filterItemsObject, R.map(lensPropToJSDate('date_taken')), flickrImageData)(flickrData))).title;
